      ...
      {filteredProjects.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8" role="list" aria-label="Lista de tus inspiraciones guardadas">
          {filteredProjects.map(project => (
            <div key={project.id} className="gradient-card rounded-3xl overflow-hidden group transition-all duration-300 hover:-translate-y-1" role="listitem" tabIndex={0}>
              <ImageWithFallback 
                ...
                <div className="flex gap-2 mt-2 overflow-hidden">
                    {project.styleVariations.slice(0, 4).map(variation => (
                        <ImageWithFallback 
                            ...
                            className="w-10 h-10 rounded-md object-cover border-2 border-gray-200"
                            loading="lazy"
                        />
                    ))}
                </div>
        ...
        <div className="text-center py-10">
            <p className="text-text-color-soft">No encontré ninguna inspiración con ese nombre.</p>
        </div>
      )}
    </div>
  );
